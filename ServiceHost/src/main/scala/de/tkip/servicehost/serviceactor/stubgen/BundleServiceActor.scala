...
import scala.collection.immutable.{Map => ImMap, List}
...
class BundleServiceActor extends ServiceActor {
  ...
  override protected def states: List[State] = List(
    ReceiveState(0, "exitcondition", Map("m1" -> Target("Subj2:6ade7atg-d3c2-46df-a3d0-c7f328e9af457", -1, -1, false, "")), Map("m1" -> 1), "", ""),
    loaduserconfig(1, "exitcondition", Map(), Map("1" -> 2), "load user config", ""),
    SendState(2, "exitcondition", Map("m10" -> Target("Subj2:6ade7atg-d3c2-46df-a3d0-c7f328e9af457", -1, -1, false, "")), Map("m10" -> 3), "", ""),
...
//          }
  ...
  case class loaduserconfig(override val id: Int, override val exitType: String, override val targets: Map[BranchID, Target], override val targetIds: Map[BranchID, Int], override val text: String, override val variableId: String) extends State("action", id, exitType, targets, targetIds, text, variableId) {

    def process()(implicit actor: ServiceActor) {
      val msg: ImMap[String, ImMap[String, JsValue]] =
        if (messageContent == "load test01") {
          ImMap(
            "gasstation" -> ImMap("fuel" -> "gasoline".toJson),
            "kiosk" -> ImMap("articles" -> Set("books", "tobaccos").toJson)
          )
        }
        else if (messageContent == "load test02") {
          ImMap(
            "gasstation" -> ImMap("fuel" -> "gasoline".toJson)
          )
        }
        else {
          log.warning("unknown messageContent: " + messageContent)
          ImMap()
        }

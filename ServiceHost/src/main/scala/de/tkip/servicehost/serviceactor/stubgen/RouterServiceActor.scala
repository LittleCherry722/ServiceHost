...
import de.tkip.vasec.{VRoute, geometric, VSinglePoint}
...
class RouterServiceActor extends ServiceActor {
  ...
  val startNodeIndex: String = "4"


  override protected def states: List[State] = List(
    ReceiveState(0, "exitcondition", Map("m8" -> Target("Subj2:c60d4f03-2110-4eeb-9db4-19db79ed5433", -1, -1, false, "")), Map("m8" -> 1), "Router: receive Destination Points", ""),
    CalculateRoutes(1, "exitcondition", Map(), Map("1" -> 2), "Calculate Routes", ""),
    SendState(2, "exitcondition", Map("m6" -> Target("Subj2:c60d4f03-2110-4eeb-9db4-19db79ed5433", -1, -1, false, "")), Map("m6" -> 3), "VAR:m6", ""),
    ExitState(3, null, Map(), Map(), null, null),
    ReceiveState(4, "exitcondition", Map("m5" -> Target("Subj2:c60d4f03-2110-4eeb-9db4-19db79ed5433", -1, -1, false, "")), Map("m5" -> 0), "Router: receive StartEnd", "")
  )
...
//          }
  ...
  case class CalculateRoutes(override val id: Int, override val exitType: String, override val targets: Map[BranchID, Target], override val targetIds: Map[BranchID, Int], override val text: String, override val variableId: String) extends State("action", id, exitType, targets, targetIds, text, variableId) {

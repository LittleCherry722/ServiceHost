...
class StubGeneratorActor extends InstrumentedActor {
  ...
  def fillInClass(classPath: String, name: String, id: String, states: Map[Int, State], messages: Map[String, String], jsonPath: String): Future[Any] = {
    ...
    for (state <- states.values) {
      state match {
        case s: ActionState => {
          ...
          text = text + (s.toString.replaceFirst("ActionState", s.text.replaceAll("\"", ""))) + ","
        }
    ...
    for (state <- states.values) {
      state match {
        case s: ActionState => {
          impementation = impementation + "\n  case class " + state.text.replaceAll("\"", "") + "(override val id: Int, override val exitType: String, override val targets: Map[BranchID, Target], override val targetIds: Map[BranchID, Int], override val text: String) extends State(\"action\", id, exitType, targets, targetIds, text) {\n"

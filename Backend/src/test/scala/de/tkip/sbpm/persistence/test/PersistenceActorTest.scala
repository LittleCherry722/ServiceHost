...
class PersistenceActorTest {
  ...
  def graph() {
    ...
    val idFuture1 = actor ? SaveGraph(Graph(None, "{}", DatabaseAccess.currentTimestamp, 1))
    val idFuture2 = actor ? SaveGraph(Graph(None, "{}", DatabaseAccess.currentTimestamp, 2))
    val id1 = Await.result(idFuture1.mapTo[Option[Int]], timeout.duration)
    val id2 = Await.result(idFuture2.mapTo[Option[Int]], timeout.duration)
    val allFuture = actor ? GetGraph()
    val oneFuture1 = actor ? GetGraph(id1)
    val oneFuture2 = actor ? GetGraph(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    val one2 = Await.result(oneFuture2.mapTo[Option[M]], timeout.duration)
    assertEquals(2, one2.get.processId)
    actor ! SaveGraph(Graph(id2, "{}", DatabaseAccess.currentTimestamp, 3))
    val oneFuture3 = actor ? GetGraph(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertEquals(3, one3.get.processId)
    actor ! DeleteGraph(id1.get)
    actor ! DeleteGraph(id2.get)
  ...
  def group() {
    ...
    val oneFuture1 = actor ? GetGroup(id1)
    val oneFuture2 = actor ? GetGroup(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    ...
    actor ! SaveGroup(Group(id2, "{}", false))
    val oneFuture3 = actor ? GetGroup(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertFalse(one3.get.isActive)
    actor ! DeleteGroup(id1.get)
    actor ! DeleteGroup(id2.get)
  ...
  def groupRole() {
    ...
    actor ! SaveGroupRole(GroupRole(1, 2))
    actor ! SaveGroupRole(GroupRole(3, 4))
    val allFuture = actor ? GetGroupRole()
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    actor ! SaveGroupRole(GroupRole(3, 4, false))
  ...
  def groupUser() {
    ...
    actor ! SaveGroupUser(GroupUser(1, 2))
    actor ! SaveGroupUser(GroupUser(3, 4))
    val allFuture = actor ? GetGroupUser()
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    actor ! SaveGroupUser(GroupUser(3, 4, false))
  ...
  def message() {
    ...
    time.setNanos(0)
    val idFuture1 = actor ? SaveMessage(Message(None, 1, 2, 1, true, "Message 1", DatabaseAccess.currentTimestamp))
    val idFuture2 = actor ? SaveMessage(Message(None, 3, 4, 1250, false, "Message 2", time))
    val id1 = Await.result(idFuture1.mapTo[Option[Int]], timeout.duration)
    val id2 = Await.result(idFuture2.mapTo[Option[Int]], timeout.duration)
    val allFuture = actor ? GetMessage()
    val oneFuture1 = actor ? GetMessage(id1)
    val oneFuture2 = actor ? GetMessage(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    ...
    actor ! SaveMessage(Message(id2, 10, 2, 1250, false, "Message 2.5", time))
    val oneFuture3 = actor ? GetMessage(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertEquals("Message 2.5", one3.get.data)
    actor ! DeleteMessage(id1.get)
    actor ! DeleteMessage(id2.get)
  ...
  def processInstance() {
    ...
    val idFuture1 = actor ? SaveProcessInstance(ProcessInstance(None, 1, 2, "{}", "ProcessInstance 1"))
    val idFuture2 = actor ? SaveProcessInstance(ProcessInstance(None, 3, 4, "{}", "ProcessInstance 2"))
    val id1 = Await.result(idFuture1.mapTo[Option[Int]], timeout.duration)
    val id2 = Await.result(idFuture2.mapTo[Option[Int]], timeout.duration)
    val allFuture = actor ? GetProcessInstance()
    val oneFuture1 = actor ? GetProcessInstance(id1)
    val oneFuture2 = actor ? GetProcessInstance(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    ...
    actor ! SaveProcessInstance(ProcessInstance(id2, 10, 2, "{}", ""))
    val oneFuture3 = actor ? GetProcessInstance(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertEquals(10, one3.get.processId)
    actor ! DeleteProcessInstance(id1.get)
    actor ! DeleteProcessInstance(id2.get)
  ...
  def relation() {
    type M = Relation
    actor ! SaveRelation(Relation(1, 2, 3, 4))
    actor ! SaveRelation(Relation(5, 6, 7, 8))
  ...
  def role() {
    ...
    val oneFuture1 = actor ? GetRole(id1)
    val oneFuture2 = actor ? GetRole(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    val one2 = Await.result(oneFuture2.mapTo[Option[M]], timeout.duration)
    assertTrue(one2.get.isActive)
    actor ! SaveRole(Role(id2, "{}", false))
    val oneFuture3 = actor ? GetRole(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertFalse(one3.get.isActive)
    actor ! DeleteRole(id1.get)
    actor ! DeleteRole(id2.get)
  ...
  def user() {
    ...
    val oneFuture1 = actor ? GetUser(id1)
    val oneFuture2 = actor ? GetUser(id2)
    val all = Await.result(allFuture.mapTo[Seq[M]], timeout.duration)
    assertEquals(2, all.length)
    val one1 = Await.result(oneFuture1.mapTo[Option[M]], timeout.duration)
    assertEquals(id1, one1.get.id)
    ...
    actor ! SaveUser(User(id2, "{}", false, 23))
    val oneFuture3 = actor ? GetUser(id2)
    val one3 = Await.result(oneFuture3.mapTo[Option[M]], timeout.duration)
    assertEquals(23, one3.get.inputPoolSize)
    actor ! DeleteUser(id1.get)
    actor ! DeleteUser(id2.get)

...
public class InternalBehaviorExample extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		try {
			...
			// req is ExecuteAction not just Action
			System.err.println("LENGTH:" + byteProto.length);
			ExecuteAction eAction = ExecuteAction.parseFrom(byteProto);
			
			System.err.println(eAction.toString());
			
			Action action = eAction.getAction();
			...
			// send request back
			resp.getOutputStream().write(byteProto);
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		Iterator it = audiBestellung.internalBehavior.getStatesMap().keySet().iterator();
		while (it.hasNext()) {
			int s = (int) it.next();
		...
		State currentState = audiBestellung.internalBehavior.getStatesMap().get(audiBestellung.internalBehavior.getCurrentState());

...
function gf_guiClearInputFields ()
{
	...
	gf_guiElementWrite(gv_elements.inputNodeChooseAgentSubject, "string", "");
...
function gf_guiDisplayNode (node)
{
	...
	if (gt_type.substr(0, 1) == "$" || gt_type == "action" || gt_type == "macro")
	{
		...
		gf_guiElementWrite(gv_elements.inputNodeChooseAgentSubject, "string", node.getChooseAgentSubject(), "");
...
/**
 * This method is used to fill a select field with all available external interface subjects.
 *
 ...
 * @returns {void}
 */
function gf_guiLoadDropDownInterfaceSubjects (elementSubject, excludeSubject)
{
...
function gf_guiLoadDropDownForNode (behavior, nodeType)
{
	...
	gf_guiElementHide(gv_elements.inputNodeChooseAgen);
	gf_guiElementHide(gv_elements.inputNodeCSubjectsO);
	gf_guiElementHide(gv_elements.inputNodeChooseAgent);
	gf_guiElementHide(gv_elements.inputNodeOptMessageO);
	gf_guiElementHide(gv_elements.inputNodeOptSubjectO);
	...
	{
		var gt_predefAction	= {subject: false, message: false, wildcard: false, conversation: false, correlationid: false, options: false, state: false, chooseagent: false};
...
function gf_guiReadNode ()
{
  ...
  var gt_variable        = gf_guiElementRead(gv_elements.inputNodeVariable, "string", "");
	...
	var gt_chooseAgentSubject	= gf_guiElementRead(gv_elements.inputNodeChooseAgentSubject, "string", "");

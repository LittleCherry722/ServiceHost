...
class SubjectActor(subject: Subject) extends Actor {
  ...
  def receive = {
    ...
    case msg @ SubjectToSubjectMessage(from, to, _) => {
      if(to == subject.subjectID){
      /**
       * the first state of the subject 2 of the TestPair 2 is receive state
       * so there is no control statement
       * in fact we need input pool to store the message
       */

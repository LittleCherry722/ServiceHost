...
object ProcessInstanceInterfaceActor {
  case class TestPairInstance(instance: Int)
}

class ProcessInstanceInterfaceActor extends Actor with HttpService {
  import ProcessInstanceInterfaceActor._
  ...
  def receive = runRoute({
    ...
    put {
      path("") {
        entity(as[TestPairInstance]) { instance =>
          processInstanceActor ! instance
          complete("Test pair changed successfully.")
        }
      } ~
      path("restart") {

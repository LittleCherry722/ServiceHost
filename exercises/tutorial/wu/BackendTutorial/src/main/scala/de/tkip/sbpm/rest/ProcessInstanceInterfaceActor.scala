...
class ProcessInstanceInterfaceActor extends Actor with HttpService {
  ...
  def receive = runRoute({
    get {
      ...
      put {
        path("") {
          entity(as[DataSetHeader]) { json =>
            processInstanceActor ! ChangeTestData(json.instance)
            complete("TestData changed")
          }
        } ~
          path("restart") {
          ...
          path("send" / IntNumber) { id =>
              entity(as[MessageHeader]) { json =>
                processInstanceActor ! SubjectToSubjectMessage(id,f(id),json.message)
